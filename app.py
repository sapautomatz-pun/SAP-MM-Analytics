# ==========================================================
# SAP AUTOMATZ - Procurement Analytics AI App (v21.0)
# ==========================================================
# ✅ Unicode-safe DejaVu font
# ✅ Color-coded KPI boxes
# ✅ GPT-4o AI + charts + Airtable portal
# ==========================================================
...
def ai_summary(prompt):
    try:
        r = client.chat.completions.create(
            model=MODEL,
            messages=[
                {"role": "system", "content": "You are an expert SAP procurement analyst writing a professional summary."},
...
def charts(df):
    ...
    if "MATERIAL" in df and "VALUE" in df:
        ...
        plt.figure(figsize=(5,5)); plt.pie(sp, labels=sp.index, autopct="%1.1f%%"); plt.title("Material Spend"); 
        p = "/tmp/mat.png"; plt.savefig(p); paths["Material Spend"] = p; st.image(p)
    if "PO_DATE" in df and "VALUE" in df:
        df["MONTH"] = df["PO_DATE"].dt.to_period("M").astype(str)
        m = df.groupby("MONTH")["VALUE"].sum()
        plt.figure(figsize=(8,4)); m.plot(marker="o", color="orange"); plt.title("Monthly Spend Trend"); plt.tight_layout()
        p = "/tmp/trend.png"; plt.savefig(p); paths["Monthly Trend"] = p; st.image(p)
    return paths

# ----------------------------------------------------------
# PDF GENERATION (Unicode-safe + KPI colors)
# ----------------------------------------------------------
class PDF(FPDF):
    def header(self):
...
def generate_pdf(ai_text, k, chart_paths):
    ...
    for label,val,thr in metrics:
        ...
        pdf.multi_cell(w-4,6,f"{label}\n{val if val is not None else 'N/A'}",align="C")
        x0 += 65
    pdf.ln(35)
    ...
    for sec,emo in sections.items():
        ...
        text = match[0].strip() if match else "N/A"
