...
with col2:
    st.markdown("""
        ...
        <p style='color:#444;margin-top:0;font-size:15px;'>
        Empowering <b>SAP and ERP procurement teams</b> with AI-driven insights.<br>
...
def calculate_kpis(df):
    """Auto-detect key numeric and vendor columns"""
...
def ai_summary(k):
    ...
    prompt = f"""You are a procurement analytics expert.
Provide a business summary for this dataset:
Records: {k['records']}
Total Spend: {k['total_spend']:,}
Date Range: {k.get('date_range','N/A')}
Top Vendor: {k.get('top_vendor','N/A')}
Summarize procurement performance, vendor dependency, cost efficiency, and improvement suggestions."""
    try:
        r = client.chat.completions.create(
            model=MODEL,
            messages=[
                {"role": "system", "content": "You are an SAP/ERP Procurement Analyst writing a professional executive summary."},
...
def generate_pdf(ai_text, k, charts, customer, key):
    ...
    pdf.cell(0,10,f"License Key: {key}",align="C",ln=True)
    pdf.cell(0,10,f"Date: {datetime.date.today().strftime('%d %b %Y')}",align="C",ln=True)
    pdf.ln(10)
    pdf.set_font("DejaVu","",11)
    pdf.cell(0,10,"Data Source: SAP / ERP Procurement Extract",align="C",ln=True)
    ...
    if not ai_text or ai_text.startswith("AI Error"):
        pdf.multi_cell(0,8,"No AI summary generated. Please verify OpenAI API key.")
    else:
